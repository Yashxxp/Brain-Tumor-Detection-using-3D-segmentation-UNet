...
def load_nifti_folder(image_dir):
    ...
    target_shape = (160, 192, 128)  # The target shape after resizing
    ...
    for fname in tqdm(filenames, desc="Loading NIfTI data"):
        ...
        # Check the shape after resizing and ensure it's consistent
        print(f"Resized shape for {fname}: {img_resized.shape}")
        if img_resized.shape != target_shape + (1,):
            print(f"Skipping {fname} due to mismatched shape.")
            continue  # Skip this image if the shape doesn't match

        # Replicate the single channel data across 4 channels
        img_resized = np.repeat(img_resized, 4, axis=-1)  # Replicate the 1 channel to 4 channels
